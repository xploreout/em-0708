...
- **View and manage classes** — access all active classes assigned to you
...
- **Search attendance history** — look up a person across all classes to see their participation record
- **Upload class resources** — attach documents (PDFs, Word, PowerPoint, Excel, images) to a class or specific session for members to reference
...
Each individual class can have its own **lead password** set by an Admin. The class leader uses this password to verify their identity before making certain changes to the class. This is separate from the role-level login password.

---

## Coworker Role

Intended for **ministry coworkers** who are assigned duties on the monthly schedule.
...
- **View church document repository** — browse and download documents uploaded by the Admin (announcements, resources, references, etc.)

### What the Coworker Role Cannot Do

- Coworkers cannot edit, add, or delete schedule entries — those actions are Admin-only
- Coworkers cannot upload documents to the repository
...
- **Manage event types** — define recurring or one-time event categories
- **Manage teams** — organize coworkers into teams
...
- **Add new classes** — create classes with details such as name, leader, location, meeting schedule, and recurrence (weekly/monthly/one-time)
- **Set the class lead password** — assign or update the verification password for an individual class leader
...
There are three system-level role passwords — one each for Class, Coworker, and Admin. These are shared passwords; everyone using the same role uses the same password.
...
To change a role password, log in as Admin, navigate to the password management section, select the role, and enter the new password.

### Class Lead Passwords

Each class can have its own individual lead password, separate from the Class role password. This password is set by the Admin and given to the class leader.
...
- Class lead passwords can be updated at any time through the class management interface
